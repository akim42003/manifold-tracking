...
Let $u \in S^{d-1}$, $c > 0$, and $v \in \mathbb{R}^d$. Decompose $v = v_\parallel u + v_\perp$ with $v_\perp \perp u$. Suppose $c + v_\parallel > 0$. Then the vector
\[
u' \;=\; \frac{c u + v}{\| c u + v \|}
\]
...
The verdict for Proposition~\ref{prop:centroid-quadratic} under this value of $L$: the interior-quadratic term dominates the boundary-linear term when $\varepsilon \ll 1/L \approx 0.09$. This is a non-trivial but narrow regime. For perturbations $\varepsilon$ of order $10^{-2}$ or smaller, the quadratic centroid-stability claim is substantive; for $\varepsilon$ of order $10^{-1}$ or larger, the linear boundary-churn term dominates and the stability claim reduces to the general $O(\varepsilon)$ bound.
...
\begin{proposition}[Centroid-pair cosine is quadratic in $\varepsilon$]
\label{prop:centroid-quadratic}
Let $\theta'$ be a perturbation of $\theta$. Assume (as above) that anchoring is preserved for all features, and that the set of features whose cluster assignment flips has size $O(\varepsilon)$ times $|\mathcal{F}|$ to leading order. Fix a cluster $k$ with fixed-point cluster $S_k \subseteq \mathcal{F}$ of size $n_k$ and centroid $c_k$ in model $\theta$. Let $c'_k$ be the corresponding centroid in $\theta'$. Then
\[
\cossim(c_k Q, \, c'_k) \;=\; 1 - O(\varepsilon^2) \;+\; O(\varepsilon)
\]
where the $O(\varepsilon^2)$ term reflects the geometric displacement of the centroid direction due to offsets that remain assigned to cluster $k$, and the $O(\varepsilon)$ term reflects the contribution of flipped features.
\end{proposition}

\begin{proof}[Proof sketch]
Decompose
\[
c'_k - c_k Q \;=\; \underbrace{ \big( \text{update from unflipped features} \big) }_{\text{first component}} + \underbrace{ \big( \text{update from flipped features} \big) }_{\text{second component}}.
\]
For unflipped features, the contribution is $\sum_{f \in S_k, \text{unflipped}} [\mathcal{O}(f; \theta') - \mathcal{O}(f; \theta) Q]$, of norm bounded by $\sum_f \alpha_f \cdot 1 \cdot 1 = O(n_k \cdot \varepsilon)$. However, the centroid is the \emph{normalized} mean; its direction is affected by the perpendicular component only, which by Lemma~\ref{lem:deflection} produces a cosine deficit quadratic in $\alpha_f$. Under an isotropy assumption that $\sum_f \delta_f^\perp$ has mean zero, the summed perturbation has norm $O(\sqrt{n_k} \cdot \varepsilon)$, and the resulting angular deflection of $c'_k$ from $c_k Q$ is $O(\varepsilon / \sqrt{n_k})$, giving a cosine deficit of $O(\varepsilon^2 / n_k)$.

For flipped features, the contribution is (at most) the number of flipped features in cluster $k$, which under hypothesis (R) from Remark~\ref{rem:margin-density} is $O(\varepsilon \cdot n_k)$. Each flipped feature contributes linearly to the centroid shift, giving a cosine deficit of $O(\varepsilon)$. In regimes where hypothesis (R) holds with small $L$, the interior (quadratic) contribution can dominate.

The stated leading behavior therefore has a quadratic contribution from the geometric deformation of the cluster's interior, plus a linear contribution from membership churn at the boundary. If hypothesis (R) fails or $L$ is large, the linear term dominates; if $L$ is small (well-separated clusters) and $\varepsilon$ is small enough relative to $L^{-1}$, the quadratic term dominates and $\cossim(c_k Q, c'_k) = 1 - O(\varepsilon^2)$.
\end{proof}

\begin{remark}
Proposition~\ref{prop:centroid-quadratic} is stated as a sketch because the precise constants depend on hypothesis (R) of Remark~\ref{rem:margin-density}, the cluster-size distribution, and the correlation structure of the perturbations across features. We leave a full treatment (along the lines of Davis--Kahan~\citep{davis_kahan_1970} or Wedin~\citep{wedin_1972} perturbation bounds for singular subspaces) to follow-up work. What the sketch establishes is the \emph{qualitative} claim: centroid deformation is \emph{quadratic} in $\varepsilon$ in the interior of the cluster, with a linear correction from boundary churn whose magnitude depends on an empirically testable property of the margin distribution.
\end{remark}
...
The perturbation analysis of Section~4 identifies two distinct stability regimes. The offset deflection angle $\alpha_f$ is linear in $\varepsilon$ with amplification $1 / \mu_f^\theta$ (Corollary~\ref{cor:offset-deflection}). The cluster-assignment flip fraction is bounded by the empirical margin CDF evaluated at $O(\varepsilon)$ (Lemma~\ref{lem:assignment-stability}); under an empirical regularity hypothesis on this CDF near zero --- hypothesis (R) of Remark~\ref{rem:margin-density} --- the flip fraction is linear in $\varepsilon$. In contrast, the centroid-pair cosine deformation is quadratic in $\varepsilon$ in the interior of clusters, with a linear boundary correction whose magnitude depends on (R) (Proposition~\ref{prop:centroid-quadratic}).

The qualitative upshot, contingent on hypothesis (R): \emph{cluster geometry is more robust than cluster membership}. Small perturbations to the parameter set produce changes in which specific features sit in which clusters at a linear rate in $\varepsilon$, but changes in where the clusters \emph{are} as geometric objects at a quadratic rate in $\varepsilon$. This provides a formal basis --- conditional on the empirical behavior of the margin CDF near zero --- for an intuition that has been implicit in our empirical analyses: averaged per-cluster statistics (centroid direction, cluster size, $d_{50}$) should be more stable across twin-trained models than per-feature assignments. Hypothesis (R) has been empirically verified on one model (Remark~\ref{rem:cdf-empirical}) with a moderate constant $L \approx 11$, which makes the quadratic-over-linear separation substantive but narrow: the regime where the quadratic centroid term dominates is $\varepsilon \ll 1/L \approx 0.09$.
